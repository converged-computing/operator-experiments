...
def gather_outputs(outdir):
    ...
    for file in recursive_find(outdir):
        ...
        while lines:
            ...
            # Flux Operator times look like this
            # FLUXTIME fluxsubmit wall time 0:44.09
            if line.startswith("FLUXTIME"):
                rest, walltime = line.strip().rsplit(" ", 1)
                timekey = (
                    rest.replace("FLUXTIME ", "").strip(" ").replace(" ", "_")
                    + "_seconds"
                )
                entry[timekey] = timestr2seconds(walltime)
                entry[f"{timekey}_raw"] = walltime

            # Flux times when debug is True
            # ƒo8zefR: 147.598s finish
            if line.startswith('ƒ'):

                if "fluxtimes" not in results:
                    results['fluxtimes'] = {}
                if "fluxvars" not in results:
                    results['fluxvars'] = {}

                _, rest = line.spllit(':', 1)
                # If we have another :, it's a parameter (not a time)
                # ƒo8zefR: complete: status=0              
                if rest.count(':') != 0:
                    stage, values = rest.split(':', 1)
                    results['fluxvars'][stage.strip()] = values.strip()                                
                else:
                    # ƒo8zefR: 147.600s release
                    seconds, stage = rest.split(' ', 1)
                    results['fluxtimes'][stage.strip()] = float(seconds.replace('s', ''))
            ...
            # This is the command with the matrix size, etc.
            if "Extra arguments are" in line:
                entry["command"] = line.split(":")[-1].strip()
                continue
