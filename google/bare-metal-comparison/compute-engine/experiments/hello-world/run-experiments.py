...
def get_parser():
    ...
    parser.add_argument(
        "--no-cpu-affinity",
        dest="no_cpu_affinity",
        action="store_true",
        default=False,
        help="Don't add flag for cpu affinity",
    )
    parser.add_argument(
        "--no-ompi",
        dest="no_ompi",
        action="store_true",
        default=False,
        help="Don't add flag for ompi=5",
    )
    return parser
...
def main():
    ...
    print("cpu-affinity: %s" % (not args.no_cpu_affinity))
    ...
    # If we remove the ompi flag
    if args.no_ompi:
        flux_options = flux_options[1:]
    ...
    for i in range(args.times):
        ...
        # This didn't work! Keeping here if we want to try again..
        # TODO check the parameters here against the operator, I always forget the difference here...
        # jobspec = flux.job.JobspecV1.from_command(command, num_nodes=args.N, num_tasks=args.cores)
        # if args.workdir is not None:
        #    jobspec.cwd = args.workdir

        # Set output and error to the log file
        # jobspec.stdout = outfile
        # jobspec.stderr = outfile
        # jobspec.environment = dict(os.environ)

        # Add mpi option flag
        # jobspec.setattr_shell_option("mpi", "openmpi@5")

        # future = flux.job.submit(handle, jobspec)
