...
wrapper=/mnt/share/data/netmark-experiment-size-90/netmark_wrapper.sh
cat ${wrapper}
chmod +x ${wrapper}
...
if [ $BATCH_TASK_INDEX = 0 ]; then
  cd /mnt/share/data/netmark-experiment-size-90
  ls
  echo "mpirun -n 90 -ppn 1 -f BATCH_HOSTS_FILE ${wrapper}"
  mpirun -n 90 -ppn 1 -f $BATCH_HOSTS_FILE ${wrapper}
else
  # Since we cannot set a barrier, this keeps the instances running
  while [ ! -f "/mnt/share/data/netmark-experiment-size-90/RTT.csv" ]; do 
    sleep 30;
  done
