...
def plot_results(df, outdir):
    ...
    # We can filter down to only those that started AFTER submission done
    subset = df[df.submit_to_completion_time.isnull() == False]
    make_plot(
        subset,
        title="Submit through Completion Time for Different Schedulers",
        tag="submit-to-completion-times",
        ydimension="submit_to_completion_time",
        xdimension="experiment",
        palette=palette,
        outdir=outdir,
        ext="png",
        plotname="submit-to-completion-times",
        hue="scheduler",
        plot_type="bar",
        xlabel="Experiment",
        ylabel="Time (seconds)",
...
def parse_data(files):
    ...
    for filename in files:
        ...
        submit_to_completion = None

        # Do we have batch to submit completion time?
        # We want to only include times for which the job started AFTER
        # this time. If the job started before, we don't know when in the range
        # of submits it started. If it started after, we know in that waiting
        # period it was also still pending.

        if "batch_done_submit_time" in times:
            bdt = datetime.strptime(
                times["batch_done_submit_time"], "%Y-%m-%d %H:%M:%S.%f"
            )
            st = datetime.strptime(times["start_time"], "%Y-%m-%d %H:%M:%S.%f")
            if st > bdt:
