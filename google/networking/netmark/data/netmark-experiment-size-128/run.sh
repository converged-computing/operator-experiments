...
wrapper=/mnt/share/data/netmark-experiment-size-128/netmark_wrapper.sh
cat ${wrapper}
chmod +x ${wrapper}
...
if [ $BATCH_TASK_INDEX = 0 ]; then
  cd /mnt/share/data/netmark-experiment-size-128
  ls
  echo "mpirun -n 128 -ppn 1 -f BATCH_HOSTS_FILE ${wrapper}"
  mpirun -n 128 -ppn 1 -f $BATCH_HOSTS_FILE ${wrapper}
else
  # Since we cannot set a barrier, this keeps the instances running
  while [ ! -f "/mnt/share/data/netmark-experiment-size-128/RTT.csv" ]; do 
    sleep 30;
  done
