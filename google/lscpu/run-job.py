...
def get_parser():
    ...
    parser.add_argument(
        "--job-name", dest="job_name", help="Name for the job", default="lscpu-job"
    )
    parser.add_argument(
        "--bucket",
        help="Bucket to save results in",
        default="netmark-experiment-bucket",
    )
    parser.add_argument(
        "--machine-type",
    ...
    parser.add_argument(
        "--tasks", help="number of batch tasks (nodes).", type=int, default=1
    )
    parser.add_argument(
    ...
    parser.add_argument("--retry-count", help="retry count", type=int, default=1)  # 1GB
    parser.add_argument(
        "--max-run-duration",
...
def batch_job(
    ...
    outdir = os.path.join(outdir, machine_type)

    # The script output directory includes outdir / machine name
    run_template = jinja2.Template(run_script)
    runscript = run_template.render({"outdir": outdir, "mount_path": mount_path})
