...
def plot_outputs(raw, not_optimized, plotname, ext="pdf", outdir="img"):
    """
    Parse results.json into dataframe and plots to save.
    """
    # Find common sizes between the two
    sizes_a = set([x['size'] for _,x in raw['meta']['jobs'].items()])
...
def get_parser():
    ...
    parser.add_argument(
        ...
        dest="not_optimized",
        default="unoptimized_results.json",
        nargs="?",
    )
    parser.add_argument(
        "-p",
