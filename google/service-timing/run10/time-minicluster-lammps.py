...
# Get tag (runtype)
if len(sys.argv) != 2:
    sys.exit("Please include the save tag (e.g., time-minicluster-lammps.py lammps)")
tag = sys.argv[1]
...
container = {
    ...
    "flux_log_level": 7,
    "resources": {
        "limits": {"cpu": 50, "memory": "448G"},
        "requests": {"cpu": 50, "memory": "448G"},
    },
    "environment": {
...
with_stdout = False
run_experiments(
    os.path.join(outdir, "lammps-no-services.json"), False, with_stdout, True
)
# run_experiments(
