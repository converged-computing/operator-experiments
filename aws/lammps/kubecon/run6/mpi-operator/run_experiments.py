...
def run(kube_client, apps, num_runs, num_run_configs, node_topology):
    ...
    for idx in range(num_run_configs):
        ...
        for rep in range(1, num_runs + 1):
            ...
            print(f"\nStarting jobid {job_id}, run {rep} of {num_runs} with the {sched} scheduler")
...
def main():
    ...
    config_lens = set()
    for app in apps:
        outdir = app['metadata']['log_dir']
        if not os.path.isdir(outdir):
            print(
                f"Output directory {outdir} does not exist. Please create it before running tests."
            )
            return
