...
large_config = [
    # These are smaller to make the time reasonable, and have complete scaling picture
    {"x": 8, "y": 8, "z": 8, "cpu_limit": 2, "tasks": 8, "size": 4, "min_size": 2},
    {"x": 8, "y": 8, "z": 8, "cpu_limit": 2, "tasks": 16, "size": 8, "min_size": 6},
    {"x": 8, "y": 8, "z": 8, "cpu_limit": 2, "tasks": 32, "size": 16, "min_size": 12},
    #    {"x": 32, "y": 16, "z": 8, "cpu_limit": 2, "tasks": 32, "size": 16, "min_size": 12},
]
...
def get_logs(uid, meta, start_time, log_dir, node_topology):
    ...
    while True:
        ...
        if len(pods.items) >= meta["params"]["min_size"] and is_running:
            break
    ...
    for b in range(batches):
        ...
        for i in range(7, 20):
            ...
            for cfg in cfgs:
                ...
                specs[uid]["start_time"] = str(start_time)
                start_times[uid] = start_time
        ...
        with multiprocessing.Pool(processes=len(batch)) as pool:
            ...
            # TODO note this freezes on the last one, and I am doing control C since
            # we are done anyway.
